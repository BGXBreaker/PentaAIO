    ...
    void load()
    {
        ...
        main_tab->set_assigned_texture(myhero->get_square_icon_portrait());
        {
            ...
            auto combo = main_tab->add_tab(myhero->get_model() + ".combo", "Combo Settings");
            {
                ...
                {
                    combo::q_range = q_config->add_slider(myhero->get_model() + ".q.range", "Q Range", 1090, 1, 1090);
                    combo::q_range->set_texture(myhero->get_spell(spellslot::q)->get_icon_texture());
...
#pragma endregion
    ...
    void on_draw()
    {
        ...
        if (q->is_ready() && draw_settings::draw_range_q->get_bool())
            draw_manager->add_circle_with_glow(myhero->get_position(), draw_settings::q_color->get_color(), combo::q_range->get_int(), 2.0f, glow_data(0.1f, 0.75f, 0.f, 1.f));
